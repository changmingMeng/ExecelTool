...
class ExcelDriver2G(ExcelDriver):
...
class ExcelDriver3G(ExcelDriver):
        ...
        for i in (0, 2):
            ...
            for r in range(sheet.nrows):
                if sheet.cell_value(r, 3) in self.targets:
                    ...
                    data = sheet.cell_value(r, 7) + sheet.cell_value(r, 8) + sheet.cell_value(r, 9) + sheet.cell_value(
                        r, 10)
                    data /= (1024 * 1024)
                    print name, date, erl, data
                    self.results.append((name, date, erl, data))
...
class ExcelDriver4G(ExcelDriver):
...
class ExcelDriverFactory(object):


    def makeED(self, filename):
        if re.search('GSM', filename):
            ed = ExcelDriver2G(filename)
        elif re.search('WCDMA', filename):
            ed = ExcelDriver3G(filename)
        else:
            ed = ExcelDriver4G(filename)
        return ed
...
if __name__ == "__main__":
    ...
    edf = ExcelDriverFactory()
    ed = edf.makeED(u"E:\projects\jinhua\WCDMA话务量&流量指标(201701月).xlsx")
